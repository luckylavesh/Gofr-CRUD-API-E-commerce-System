...
func Test_Create(t *testing.T) {
	ctx, mock := newMock(t)
	product := &model.Product{ID: 1, Name: "SAMPLE-NAME", Price: 99, Category: "clothing"}
	...
	testCases := []struct {
		...
		{desc: "success case", input: &model.Product{ID: 1, Name: "SAMPLE-NAME", Price: 99, Category: "clothing"},
			dbMock: []interface{}{
	...
	testCases := []struct {
		...
		{desc: "success case", input: 1,
			dbMock: []interface{}{mock.ExpectQuery(getByIDQuery).
				WillReturnRows(sqlmock.NewRows([]string{"id", "name", "price", "category"}).AddRow(1,
					"SAMPLE-NAME", 99, "clothing"))},
			expOutput:   &model.Product{ID: 1, Name: "SAMPLE-NAME", Price: 99, Category: "clothing"},
			expectedErr: nil},
