...
func newMock(t *testing.T) (gofrLog.Logger, *store.MockProduct) {
...
func createContext(method string, params map[string]string, product interface{}, logger gofrLog.Logger, t *testing.T) *gofr.Context {
...
func Test_Create(t *testing.T) {
	...
	testCases := []struct {
		...
		{"success case", product, []*gomock.Call{
			mockStore.EXPECT().Create(gomock.AssignableToTypeOf(&gofr.Context{}), &product).Return(&product, nil).Times(1),
		}, &product, nil},
		{"failure case", product, []*gomock.Call{
			mockStore.EXPECT().Create(gomock.AssignableToTypeOf(&gofr.Context{}), &product).Return(nil, errors.Error("test error")).Times(1),
...
func Test_GetByID(t *testing.T) {
	...
	testCases := []struct {
		...
		{"success case", "1", []*gomock.Call{
			mockStore.EXPECT().GetByID(gomock.AssignableToTypeOf(&gofr.Context{}), 1).Return(&product, nil).Times(1),
		}, &product, nil},
		{"failure case", "1", []*gomock.Call{
			mockStore.EXPECT().GetByID(gomock.AssignableToTypeOf(&gofr.Context{}), 1).Return(nil, errors.EntityNotFound{Entity: "product", ID: "1"}).Times(1),
		}, nil, errors.EntityNotFound{Entity: "product", ID: "1"}},
...
func Test_Update(t *testing.T) {
	...
	testCases := []struct {
		...
		{"success case", "1", product, []*gomock.Call{
			mockStore.EXPECT().Update(gomock.AssignableToTypeOf(&gofr.Context{}), &product).Return(&product, nil).Times(1),
		}, &product, nil},
		{"failure case", "1", product, []*gomock.Call{
			mockStore.EXPECT().Update(gomock.AssignableToTypeOf(&gofr.Context{}), &product).Return(nil, errors.Error("test error")).Times(1),
		}, nil, errors.Error("test error")},
